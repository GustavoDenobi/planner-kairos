...
import { eventDisplayTitle, isEventCancelled } from '@/domain/agenda';
import { EVENT_CANCELLED_LABEL } from '@/ui/features/agenda/agenda-labels';
...
export function AgendaEventCard({ orgSlug, event, variant = 'list' }: AgendaEventCardProps) {
  ...
  return (
    <Link
      ...
      className={`block max-w-full overflow-hidden rounded-xl border border-border bg-surface px-4 py-3 transition-colors hover:bg-bg ${cancelled ? 'opacity-60' : ''}`}
    >
      <div className="min-w-0 max-w-full">
        <div className="flex items-center justify-between gap-3">
          <p className={`min-w-0 truncate font-medium text-text ${cancelled ? 'line-through' : ''}`}>
            {title}
          </p>
